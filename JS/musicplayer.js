$(function () {
  ...
  function initPlayer() {
    ...
    playPreviousTrackButton.on("click", function () {
      selectTrack(-1);
      if (window.setTopLayerIndex) {
        window.setTopLayerIndex(currIndex);
      }
    });
  ...
  // Global function to play a specific track
  window.playTrack = function(trackIndex) {
    if (trackIndex >= 0) {
      currIndex = (trackIndex % trackUrl.length) - 1; // Use modulo to loop tracks, adjust because selectTrack increments
      selectTrack(1);
    }
  };
